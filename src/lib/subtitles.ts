...
export class Subtitle {
  ...
  parse(raw: string) {
    ...
    while (pos < text.length) {
      ...
      let [startMs, endMs] = [this.parseTimestamp(timestamp[0]), this.parseTimestamp(timestamp[1])];
      pos += 1;
      let out = "";
  ...
  parseTimestamp(s: string) {
    ...
    let [hh, mm, [ss, ms]] = [this.parseNumber(parts[0]), this.parseNumber(parts[1]), this.parseSeconds(parts[2])];
    ms += ss * 1000;
    ms += mm * 60 * 1000;
    ms += hh * 60 * 60 * 1000;
    return ms;
...
`;
    const CUES: Cue[] = [
      { startMs: 136612, endMs: 139376, text: "Senator, we're making\nour final approach into Coruscant." },
